...
public class BlackJack {
    ...
    /**
     * Main method of BlackJack. Sets up program and starts loop.
     * @return void
    ...
    /**
     * Click handler, runs most logic relating to button presses
     * @param button The JButton clicked
     * @return void
     * @author William Bohonak
     * @author David Kostransky
     */
    ...
    public static void click(JButton button) {
        ...
        switch (text) {
            ...
            //These buttons dont use slow print because it is annoying
            case "Bet $1":
                if (playerMoney >= 1) {
                    betAmount += 1;
                    playerMoney -= 1;
                    System.out.println("You have bet $" + betAmount + " in total...");
            ...
            case "All In":
                if (playerMoney != 0) {
                    ...
                    playerMoney = 0;
                    System.out.println("You have bet everything... Good luck...");
                } else {
                    System.out.println("You're already all in, stop clicking this button...");
                }
                totalBet.setText("Current Bet : $" + betAmount);
                totalMoney.setText("Current Balance: $" + playerMoney);
                break;
            ...
            case "Get a Loan":
                playerMoney += 1000;
                playerDebt += 1000;
                System.out.println("You took out a loan for $1000...You can always win it back later...");
    ...
    /**
     * Prints a message by letter with delays
     * @param strMessage String message to be printed
     * @return void
     * @author William Bohonak
     * @author David Kostransky
     */
     ...
     * @param time How long to wait (int)
     * @return void
     * @author William Bohonak
     * @author David Kostransky
     */
     ...
     * @param pane  JPanel of the JFrame
     * @return void
     * @author William Bohonak
     * @author David Kostransky
     */
    ...
    public static void genMenu(JFrame frame, JPanel pane) {
        ...
        //Action Listener, no I will not be using lambdas
        ...
        // ===== SETTINGS button =====
        //Removed cause we dont need it
        // JButton settingsButton = new JButton("SETTINGS");
        // settingsButton.setBounds(250, 300, 100, 50);
    ...
    /**
     * Generates all game objects and buttons
     * @return void
     * @author William Bohonak
     * @author David Kostransky
     */
    ...
    public static void genGame() {
        ...
        // ===== Visual setup =====
        //Player Cards
        JButton pCard0 = drawRect(userHand.get(0).getSuit()+userHand.get(0).getName()+userHand.get(0).getSuit(), 0, 400, 100, 125);
        JButton pCard1 = drawRect(userHand.get(1).getSuit()+userHand.get(1).getName()+userHand.get(1).getSuit(), 100, 400, 100, 125);
        playerHandValueLabel = new JLabel("Player hand value: " + player.getHandValue());
        playerHandValueLabel.setBounds(0, 350, 200, 50);
        contentPane.add(playerHandValueLabel);

        //Dealer Cards
        JButton dCard0 = drawRect(dealerHand.get(0).getSuit()+dealerHand.get(0).getName()+dealerHand.get(0).getSuit(), 0, 0, 100, 125);
        hiddenDealerCard = drawRect("?", 100, 0, 100, 125);
        dealerHandValueLabel = new JLabel("Dealer hand value: " + dealerHand.get(0).getValue() + " + ?");
        ...
        // ===== Reset Betting ====
        betAmount = 0;
        //This is very bad no good bad code
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (gameDeck.getCards().size() > 0) {
                    ...
                    JButton pCardNew = drawRect(
                            userHand.get(userHand.size() - 1).getSuit()+
                    ...
                    //Going to move this logic over to the nextTurn method
                    playerHandValueLabel.setText("Total hand value: " + player.getHandValue());
                }
                else {
                    slowPrint("Deck is empty...");
                }
            }
        });

        // ===== Finish Turn button =====
        //We really should make a method for button logic but that sounds like a lot of work
        ...
        //Turns out lambdas are way faster than making a action handler normally
    ...
    /**
     * Runs logic that ends a round of play and starts the next one
     * @return void
     * @author William Bohonak
     * @author David Kostransky
     */
    ...
    public static void nextTurn() {
        ...
        contentPane.repaint();
        //Add new card
        drawRect(dealerHand.get(1).getSuit()+dealerHand.get(1).getName()+dealerHand.get(1).getSuit(), 100, 0, 100, 125);
        dealerHandValueLabel.setText("Dealer Hand Value: " + (dealer.getHandValue()));
        ...
        //Bad code but way faster than making a timer or a dedicated update method
        ...
        int count = 0;
        while (dealer.getHandValue() < 17 && gameDeck.getCards().size() > 0) {
            count ++;
            gameDeck.drawCard(1, dealerHand);
            dealer.updateHand(dealerHand);
            dealerHandValueLabel.setText("Dealer Hand Value: " + (dealer.getHandValue()));
            drawRect(dealerHand.get(dealerHand.size()-1).getSuit()+dealerHand.get(dealerHand.size()-1).getName()+dealerHand.get(dealerHand.size()-1).getSuit(), 100 + (100*count), 0, 100, 125);
            contentPane.paintImmediately(contentPane.getBounds()); // Force UI update
            sleep(2000);
        }
        sleep(2000);
    ...
    /**
     * Creates a JButton without an action handler
    ...
    // ===================================================
    // 🔹 DRAW LABEL — creates a JLabel (text) on screen
    // ===================================================
    //I completely forgot about this tbh
    //Don't think it got called a single time
    
    /**
     * Creates a JLabel object
     * @param  name Name of the JLabel
     * @param xPos The x position of the JLabel
     * @param yPos The y position of the JLabel
     * @param width The width of the JLabel
     * @param height The length of the JLabel
     * @return JLabel
     * @author William Bohonak
     * @author David Kostransky
     */
    
    public static JLabel drawLabel(String name, int xPos, int yPos, int width, int height) {
        JLabel text = new JLabel(name);
        text.setBounds(xPos, yPos, width, height);
